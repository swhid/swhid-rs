...
> **Status: implemented.** This fix now lives in `src/git.rs` as
> `extract_tag_extra_headers_and_message()`, wired into both `release_from_git_impl`
> and `release_from_git_with_config_impl`. Regression coverage:
> `test_signed_release_swhid` in `tests/git.rs`. This document is retained as design
> rationale. (Note: the trunk uses the type-level `HashConfig<H, E>` pipeline; any
> `Box<dyn ...>` API shown in sibling design notes reflects an earlier exploration.)
