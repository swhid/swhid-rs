...
/// Build SWHID object header bytes: `<type> <len>\0`
///
/// This implements the object header format specified in SWHID,
...
/// Hash content data according to SWHID v1.2 specification (backward compatibility).
///
...
///
/// For new code, use `hash_content_with` with an explicit hash function.
pub fn hash_content(data: &[u8]) -> [u8; 20] {
    hash_swhid_object("blob", data)
}

/// Hash arbitrary SWHID object given its type and payload bytes (backward compatibility).
///
/// This implements the SWHID v1.2 object hashing algorithm for any
/// object type (blob, tree, commit, tag, snapshot) using SHA1.
///
/// For new code, use `hash_swhid_object_with` with an explicit hash function.
...
/// Hash content data using the specified hash function.
///
/// This is the new API that supports pluggable hash functions.
pub fn hash_content_with(data: &[u8], hasher: &dyn HashFunction) -> Vec<u8> {
    hash_swhid_object_with("blob", data, hasher)
}

/// Hash arbitrary SWHID object given its type and payload bytes using the specified hash function.
///
/// This is the new API that supports pluggable hash functions.
pub fn hash_swhid_object_with(typ: &str, payload: &[u8], hasher: &dyn HashFunction) -> Vec<u8> {
    let header = swhid_object_header(typ, payload.len());
...
mod tests {
    ...
    #[test]
    fn hash_content_with_sha1() {
        let hasher = Sha1Hash::new();
        let digest = hash_content_with(b"test", &hasher);
        assert_eq!(digest.len(), 20);
        // Should match the old hash_content function
        let old_digest = hash_content(b"test");
        assert_eq!(digest, old_digest);
    }

    #[test]
    fn hash_content_with_sha256() {
        let hasher = Sha256Hash::new();
        let digest = hash_content_with(b"test", &hasher);
        assert_eq!(digest.len(), 32);
        // SHA256 produces different hash than SHA1
        let sha1_digest = hash_content_with(b"test", &Sha1Hash::new());
        assert_ne!(digest, sha1_digest);
    }

    #[test]
    fn hash_swhid_object_with_different_hashers() {
        let data = b"test data";
        let sha1_hasher = Sha1Hash::new();
        let sha256_hasher = Sha256Hash::new();

        let sha1_hash = hash_swhid_object_with("blob", data, &sha1_hasher);
        let sha256_hash = hash_swhid_object_with("blob", data, &sha256_hasher);
