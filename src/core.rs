...
impl ObjectType {
    /// Convert object type to its SWHID tag string representation.
    ///
    /// Returns the tag used in SWHID identifiers:
    /// - `Content` → `"cnt"`
    /// - `Directory` → `"dir"`
    /// - `Revision` → `"rev"`
    /// - `Release` → `"rel"`
    ...
    /// Parse object type from its SWHID tag string representation.
    ///
    ...
    /// ```
    /// use swhid::ObjectType;
    /// assert_eq!(ObjectType::from_tag("cnt")?, ObjectType::Content);
    /// assert_eq!(ObjectType::from_tag("dir")?, ObjectType::Directory);
    /// # Ok::<(), swhid::SwhidError>(())
    /// ```
...
///
/// **Serialization Format Semantics**:
/// - **Hex is canonical**: The `Display` trait and `FromStr` implementation
///   use hex encoding as the canonical format for SWHID identifiers.
/// - **Alternative formats are presentation-only**: Other serialization formats
///   (base64, base32, z85, etc.) can be used for display/storage via
///   `to_string_with()` and `parse_with()`, but hex remains the standard.
/// - **Digest bytes are independent**: The serialization format does NOT affect
///   the digest bytes themselves, only how they are encoded for display.
...
impl Swhid {
    ...
    /// Get the object type of this SWHID.
    ///
    /// # Examples
    ///
    /// ```
    /// use swhid::{Content, ObjectType};
    ...
    pub fn digest_hex(&self) -> String {
        // Hex encoding should never fail, so unwrap is safe
        HexSerializer::new().encode(&self.digest).unwrap()
    }
    ...
    /// ```
    pub fn to_string_with(&self, serializer: &dyn DigestSerializer) -> Result<String, SwhidError> {
        let encoded = serializer.encode(&self.digest)?;
        Ok(format!(
            "swh:{}:{}:{}",
            self.version.as_str(),
...
mod tests {
    ...
    fn swhid_parse_with_z85() {
        ...
        let z85_str = swhid.to_string_with(&Z85Serializer::new()).unwrap();
